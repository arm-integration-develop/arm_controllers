...
public:
  void init(ros::NodeHandle& controller_nh, std::vector<std::string> joint_names)
  {
    ...
    state_publisher_.reset(
        new realtime_tools::RealtimePublisher<control_msgs::JointTrajectoryControllerState>(controller_nh, "state", 1));
  ...
  void updateDesiredStates(const ros::Time& sample_time, const Trajectory* const traj)
  {
    ...
    {
      const auto segment =
          trajectory_interface::sample((*traj)[joint_index], sample_time.toSec(), desired_joint_state_);
