...
bool DeltaController::init(hardware_interface::RobotHW * robot_hw, ros::NodeHandle & controller_nh)
{
  ...
  for (const auto & joint_name : joint_names_) {
    Joint j{
      .name_ = joint_name, .position_ctrl_ = new effort_controllers::JointPositionController()};
    ros::NodeHandle nh_position(nh_active, joint_name);
    j.position_ctrl_->init(effort_joint_interface, nh_position);
    joints_.push_back(j);
    jnt_states_.push_back(
      robot_hw->get<hardware_interface::JointStateInterface>()->getHandle(joint_name));
  }
  if (use_gazebo_) {
  ...
  cmd_subscriber_ = controller_nh.subscribe<geometry_msgs::PointStamped>(
    "command", 1, &DeltaController::commandCB, this);
...
void DeltaController::publishVel(
